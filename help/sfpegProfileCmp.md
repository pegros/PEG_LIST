...
appropriate **sfpegProfile\_\_mdt** custom metadata record in the `Profile Configuration` property.<br/>
...
The **sfpegProfile\_\_mdt** custom metadata provides most if not all configuration for the
...
- `Profile Actions` enables to add a **[sfpegActionBarCmp](/help/sfpegActionBarCmp.md)** component
  - It should contain the name of the **sfpegAction\_\_mdt** custom metadata record to be used
...
- `Profile Details` configures the content of the details section, as a JSON object with the following properties
  - `variant`: display variant, which should have one of the following possible values
